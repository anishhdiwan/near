...
job_idx=$(python ~/thesis_background/IsaacGymEnvs/utils/plan_cmds.py --run_type=ablation --num_runs=1)
echo "Job assigned a row ID ${job_idx}"
echo "-----------"

ncsn_output=$(python ~/thesis_background/IsaacGymEnvs/isaacgymenvs/cfg/ablation_generator.py --model=ncsn --job_idx=${job_idx})
